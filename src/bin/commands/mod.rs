...
async fn list_cron_jobs() -> Result<()> {
    ...
    for key in cron_keys {
        if let Ok(job_data) = conn.hgetall::<_, HashMap<String, String>>(&key).await {
            ...
            let name = job_data.get("name").unwrap_or(&"unknown".to_string());
            let cron_expression = job_data.get("cron_expression").unwrap_or(&"unknown".to_string());
            let enabled = job_data.get("enabled").unwrap_or(&"false".to_string()) == "true";
            let next_run = job_data.get("next_run").unwrap_or(&"unknown".to_string());
